...
def train(hyp,  # path/to/hyp.yaml or hyp dictionary
    ...
    train_loader, dataset = create_dataloader(train_path, imgsz, batch_size // WORLD_SIZE, gs, single_cls,
                                              ...
                                              prefix=colorstr('train: '), shuffle=True, mask_pad=data_dict['ignore_index'])
    ...
    if RANK in [-1, 0]:
        val_loader = create_dataloader(val_path, imgsz, batch_size // WORLD_SIZE * 2, gs, single_cls,
                                       ...
                                       prefix=colorstr('val: '), mask_pad=data_dict['ignore_index'])[0]
    ...
    for epoch in range(start_epoch, epochs):  # epoch ------------------------------------------------------------------
        ...
        optimizer.zero_grad()
        for i, (imgs, msks, targets, paths, _) in pbar:  # batch -------------------------------------------------------------
            ...
            if RANK in [-1, 0]:
                ...
                pbar.set_description(('%10s' * 2 + '%10.4g' * 6) % (
                    f'{epoch}/{epochs - 1}', mem, *mloss, mSegloss, targets.shape[0], imgs.shape[-1]))
        ...
        if RANK in [-1, 0]:
            ...
            if not noval or final_epoch:  # Calculate mAP
                results, val_iou, maps, _ = val.run(data_dict,
            ...
            if fi > best_fitness:
                best_fitness = fi
            log_vals = list(mloss) + list(mSegloss) + list(results) + [val_iou] + lr
...
def parse_opt(known=False):
    ...
    parser.add_argument('--cache', type=str, nargs='?', const=False, help='--cache images in "ram" (default) or "disk"')
